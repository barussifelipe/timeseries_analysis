...
Engel's ARCH model was revolutionary, but had a problem. The uncertainty of a market is called volatility, which represents the standard deviation of variance, volatility = $\sqrt{\sigma^2}$. Volatility often stays elevated for many periods and in order to ARCH address this behaviour it was needed many past squared schocks and conseqeuently many parameters. Tim Bollerslev, in 1986, addresseed this limitation by creating GARCH (Generalized Autoregressive Conditional Heterosckedastic), where he added past variance values in order to let the shock effect persist without adding a long list of past squared shocks. 
