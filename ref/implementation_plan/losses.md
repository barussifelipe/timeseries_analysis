...
For the later volatility model, apply a strictly positive lower floor to its **variance forecasts** before QLIKE and evaluation. Choose and record the floor from that estimator's training target scale after selecting the target; keep it fixed for validation and test. This only limits predictions below the floor, with no upper cap. If the model outputs volatility, apply the equivalent square-root floor in volatility units before converting to variance. Do not silently floor actual targets or forecasts inside `variance_metrics`; the model's forecast transform must be explicit and shared across evaluations.
