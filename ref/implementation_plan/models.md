...
9. Define GARCH(1,1) from supplied ω, α, β, return residual history, and initial conditional variance. Return volatility as the square root of the next conditional variance; assess fitted parameters and residuals later.
...
2. Define one forecast record with ticker, forecast date, target, prediction, estimator, horizon, and model name. Compare only common valid observations. Convert every prediction to the selected positive variance units before QLIKE/MSE/MASE; document any positivity transform and back-transformation.
...
| `models/garch.py` | GARCH(1,1) from supplied ω, α, β, return residuals, and initial variance; output square-root volatility. |
